/**
 * Created by mithilesh on 9/5/16.
 */
...
dhis2API.trackedEntityInstance.prototype.POST = function(successCallback,errorCallback,index,lookUpIndex,headers){//header added by ifhaam
...
dhis2API.enrollment.prototype.excelImportPopulatorTemp = function (header,data,tei){
     
    if (tei){
        this.orgUnit = tei.organisationUnit;
        this.tei = tei.trackedEntityInstance;
    }
...
dhis2API.enrollment.prototype.POST = function(successCallback,errorCallback,index,header){//header added by ifhaam
...
/*Events*/
dhis2API.events = function(){
    this.eventsList = [];
}

dhis2API.events.prototype.populate = function(eventspara){
    this.eventsList = eventspara;
}

dhis2API.events.prototype.getAPIObject = function(){
    var eventsArray = [];
    for(key in this.eventsList){
        eventsArray.push(this.eventsList[key].getAPIObject()); 
    }
    var eventsjson = {
        events : eventsArray
    }
...
dhis2API.events.prototype.POST = function(successCallback,index){
    var events = this.getAPIObject()
    var def = $.Deferred();
...
dhis2API.event.prototype.excelImportPopulatorTemp = function(header,data,tei){
    if(tei){
        this.tei = tei.trackedEntityInstance;
        this.orgUnit = tei.orgUnit;
    }
...
dhis2API.event.prototype.POST = function(successCallback,errorCallback,index,header){
