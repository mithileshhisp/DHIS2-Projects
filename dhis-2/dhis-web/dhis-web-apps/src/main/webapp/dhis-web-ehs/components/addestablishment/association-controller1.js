    ...
    function($rootScope,
        ...
        $scope.close = function () {
            $timeout(function() {
                ...
                console.log($scope.trackedEntityInstance1);
            ...
            $modalInstance.close( $scope.trackedEntityInstance1);
        };
        ...
        /*    AjaxCalls.getTEIbyId(trackedEntityInstance.id).then(function(tei){
                AjaxCalls.getEventbyId($modalInstance.selectedEvent.event).then(function(event){
                    if ($modalInstance.areMultipleAssociationAllowed &&
                        $modalInstance.areMultipleAssociationAllowed != "true" &&
                        event.eventMembers) {
                        // Allow only 1 association per tracked entity
                        for (var i=0;i<event.eventMembers.length;i++){
                            if (event.eventMembers[i].trackedEntity == trackedEntityInstance.type){
                                event.eventMembers[i] = tei;
                                flag = true ;
                            }
                        }
                    }
        ...
        $scope.establishmentsProgram = 'tITlMGNJTbJ';
            ...
            if (angular.isObject($scope.selectedOrgUnit)) {
                ProgramFactory.getProgramsByOu($scope.selectedOrgUnit, $scope.selectedProgram).then(function(response){
                    for(var i=0; i<response.programs.length; i++){
                        if(response.programs[i].id === $scope.establishmentsProgram ){
