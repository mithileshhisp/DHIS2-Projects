...
.service('DashboardLayoutService', function($http, DHIS2URL) {
    ...
    w.relationshipWidget = {title: 'relationships', view: "components/relationship/relationship.html", show: true, expand: true, parent: 'biggerWidget', order: 4};
    w.reportWidget = {title: 'report', view: "components/report/tei-report.html", show: true, expand: true, parent: 'biggerWidget', order: 5};
    ...
    w.messagingWidget = {title: 'messaging', view: "components/messaging/messaging.html", show: false, expand: true, parent: 'smallerWidget', order: 4};
    w.associationWidget = {title: 'association', view: "ehs/association/association.html", show: true, expand: true, parent: 'smallerWidget', order: 5};
...
.service('RegistrationService', function(TEIService, $q, $translate){
    return {
        ...
        processForm: function(existingTei, formTei, originalTei, attributesById, countryName, districtName, communityName ){
            ...
            for(var k in attributesById){
                ...
                {
                    //Country
                    if(  attributesById[k].id === 'kbBDKjtFNEp' ) {

                        formTei[k] = countryName;
                        console.log( " Country Name -- " + countryName );
                    }

                    //District
                    if( attributesById[k].id === 'DmZxOpX64Sn')
                    {
                        formTei[k] = districtName;
                        console.log( " District Name -- " + districtName );
                    }
                    // City/Community
                    if( attributesById[k].id === 'fOHkLJDVm18')
                    {
                        formTei[k] = communityName;
...
.service('EventUtils', function(DateUtils, CommonUtils, PeriodService, CalendarService, CurrentSelection, $translate, $filter, $rootScope, orderByFilter){
    ...
    return {
        createDummyEvent: function(eventsPerStage, tei, program, programStage, orgUnit, enrollment, eventOrgUnit){
            ...
            var dummyEvent = {trackedEntityInstance: tei.trackedEntityInstance, 
                              ...
                              orgUnit: eventOrgUnit.id,
                              orgUnitName: eventOrgUnit.displayName,
