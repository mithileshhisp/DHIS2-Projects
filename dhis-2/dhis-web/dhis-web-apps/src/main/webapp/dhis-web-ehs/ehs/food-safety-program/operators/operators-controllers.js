...
trackerCapture.controller('OperatorsSelectionController',
        function($rootScope,
                 $scope,
                 $modal,
                 $location,
            ...
            $scope.key  = $scope.url.substring($scope.url.indexOf("?")+1,$scope.url.length);

            $scope.establishmentid="tITlMGNJTbJ";
            $scope.waterestablishmentid="JlYxDLMZJhK";
            ...
            $scope.$watch('selectedOrgUnit', function() {
                if( angular.isObject($scope.selectedOrgUnit)){
                    ...
                    //Labels
                    $scope.programNameLevel = $translate.instant('food_safety_operators');
            ...
            //                ProgramAndTEIService.getTrackedEntityInstancesByProgram( $scope.establishmentsProgran ).then(function (teis) {
                    ...
                    ProgramFactory.getProgramsByOu($scope.selectedOrgUnit, $scope.selectedProgram).then(function(response){
                        ...
                        for(var i=0; i<response.programs.length; i++){
                            if(response.programs[i].id === $scope.operatorsProgram ){
                                //$scope.programs = response.programs[i];
                                ownerPrograms.push(response.programs[i]);
