...
window.onload=function(){
	jQuery('#latestDataFormDiv').dialog({autoOpen: false});	
}


// Load Latest Data
function loadLatestAnonymousDataForm()
{

	jQuery('#latestDataFormDiv').dialog('destroy').remove();
	jQuery('<div id="latestDataFormDiv">' ).load( 'loadLatestAnonymousData.action' ).dialog({
		title: " Latest Data ",
		maximize: true,
		closable: true,
		modal:true,
...
function saveDataInPatientDataValue()
{
	//alert( "inside save" );
	
	$.ajax({
	type: "POST",
	url: 'saveDataInPatientDataValue.action',
	data: getParamsDataForDiv('latestDataFormDiv'),
...
function getParamsDataForDiv( latestDataFormDiv )
{
	var params = '';
	
	jQuery("#" + latestDataFormDiv + " :input").each(function()
		{
