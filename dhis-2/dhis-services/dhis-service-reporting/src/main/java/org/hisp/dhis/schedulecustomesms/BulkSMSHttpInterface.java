...
public class BulkSMSHttpInterface
{
    ...
    /*****
     * this constructor takes the username , password and sendername from a file
     * a file
     **********/
    /*
     ...
     * 
     * properties.load( new FileReader( System.getenv( "DHIS2_HOME" ) +
     * File.separator + "hibernate.properties" ) ); username = getUsername();
     * password = getPassword(); senderName = getSenderName(); }
     * 
     * 
     * public BulkSMSHttpInterface( String username, String password, String
     * senderName ) { this.username = username; this.password = password;
     * this.senderName = senderName; }
     */
    // getter
    /*
     * public String getUsername() { return properties.getProperty(
     * "bsms.username" ); }
     * 
     * public String getPassword() { return properties.getProperty(
     * "bsms.password" ); }
     * 
     * public String getSenderName() { return properties.getProperty(
     * "bsms.sender" ); }
     */

    // sending message to single mobile no
    public String sendMessage( String message, String phoneNo )
        throws MalformedURLException, IOException
    {
        ...
        // "&sendername=" + senderName + "&mobileno=" + phoneNo + "&message=" +
        // message;

        data = "token=" + token + "&from=" + from + "&to=" + phoneNo + "&text=" + message;

        // data = "username=" + username + "&password=" + password + "&to=" +
        // phoneNo + "&from=" + senderName + "&text=" + message;

        // http://myvaluefirst.com/smpp/sendsms?username=nrhmhttp&password=nrhm1234&to=9643208387&from=NRHMHR&text=hi

        // this link is used for sending sms(there are different links for
        // different functions.refer to the api for more details)
        // url_string =
        // "http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?";
        ...
        throws UnsupportedEncodingException
    {
        ...
        try
        {
             ...
             * 
             * message = gujratiTranslationMap.get( "PERFECT_MESSAGE" ) + " " +
             * gujratiTranslationMap.get( "male" ) + "(" + tempMessage[0] + ","
             * + tempMessage[1] + "," + tempMessage[2] + ")" +
             * gujratiTranslationMap.get( "female" ) + "(" + tempMessage[3] +
             * "," + tempMessage[4] + "," + tempMessage[5] + ")" +
             * gujratiTranslationMap.get( "sideEffect" ) + "(" + tempMessage[6]
             * + ") " + smsDate;
             * 
             * String user = "username=" + "harsh.atal@gmail.com"; String hash =
             * "&hash=" + "04fa1b5546432e99162704a7025403879d589271"; message =
             * "&message=" + message; String sender = "&sender=" + "TXTLCL";
             * 
            ...
            /*
             * username = "hispindia"; password = "hisp1234"; senderName =
             * "HSSPIN";
             * 
             * data = "username=" + username + "&password=" + password +
             * "&sendername=" + senderName + "&mobileno=" + phoneNo +
             * "&message=" + message; url_string =
             * "http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?";
             */
            ...
            data = "username=" + username + "&password=" + password + "&smsservicetype=" + smsservicetype + "&content="
                + finalmessage + "&mobileno=" + phoneNo + "&senderid=" + senderid;

            url_string = "http://msdgweb.mgov.gov.in/esms/sendsmsrequest?";
            /*
             * String token = "2Ca4N06OnVfBBl0BMemc"; String from = "infoSMS";
             * data = "token=" + token + "&from=" + from + "&to=" + phoneNo +
             * "&text=" + message;
             */

            // Send data
            // HttpURLConnection conn = (HttpURLConnection) new URL( url_string
            // ).openConnection();
             ...
             * ).openConnection(); conn.setDoOutput( true );
             * conn.setRequestMethod( "POST" ); conn.setRequestProperty(
             * "Content-Length", Integer.toString( data.length() ) );
             * conn.getOutputStream().write( data.getBytes( "UTF-8" ) ); final
             * BufferedReader rd = new BufferedReader( new InputStreamReader(
             * conn.getInputStream() ) ); final StringBuffer stringBuffer = new
             * StringBuffer(); String line; while ( (line = rd.readLine()) !=
             * null ) { stringBuffer.append( line ); }
             * 
             * rd.close();
             * 
    ...
    public String sendMessages( String message, List<String> phonenos )
        throws MalformedURLException, IOException
    {
        ...
        data = "username=" + username + "&password=" + password + "&sendername=" + senderName + "&mobileno=" + phoneNo
            + "&message=" + message;
    ...
    public List<String> getUsersMobileNumber( Integer organisationUnitId )
    {
        ...
        // List<OrganisationUnit> orgUnitList = new ArrayList<OrganisationUnit>(
        // organisationUnitService.getOrganisationUnitBranch( organisationUnitId
        // ) );
    ...
    // sending message to single mobile no
    /*
     * public String sendHindiMessage( String message, String phoneNo ) throws
     * MalformedURLException, IOException { if (message==null || phoneNo==null)
     * { return "either message or phone no null"; }
     * 
     * else if (message.equalsIgnoreCase( "") || phoneNo.equalsIgnoreCase( "") )
     * { return "either message or phone no empty"; } username = "PHD25PGIMER";
     * password = "sph@25"; String senderId = "PGIMER"; String smsservicetype =
     * "unicodemsg";
     * 
     * String response = null; String encryptedPassword; String genratedhashKey
     * = hashGenerator(username, senderId, message, secureKey); HttpClient
     * client = new HttpClient(); PostMethod method = new
     * PostMethod("http://msdgweb.mgov.gov.in/esms/sendsmsrequest"); try {
     * 
     ...
     * 
     * 
     * username = "hispindia"; password = "hisp1234"; senderName = "HSSPIN";
     * 
     * String token = "2Ca4N06OnVfBBl0BMemc"; String from = "infoSMS";
     * 
     * //Populating the data according to the api link
     ...
     * message;
     * 
     * data = "token=" + token + "&from=" + from + "&to=" + phoneNo + "&text=" +
     * message;
     * 
     * //data = "username=" + username + "&password=" + password + "&to=" +
     * phoneNo + "&from=" + senderName + "&text=" + message;
     * 
     * //http://myvaluefirst.com/smpp/sendsms?username=nrhmhttp&password=nrhm1234
     * &to=9643208387&from=NRHMHR&text=hi
     * 
     * //this link is used for sending sms(there are different links for
     * different functions.refer to the api for more details) //url_string =
     * "http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?"; url_string =
     * "http://api.sparrowsms.com/v2/sms/?";
     * 
     * //url_string = "https://myvaluefirst.com/smpp/sendsms?";
     * 
     * //
     * http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?username=hispindia
     * &password=hisp1234&sendername=HSSPIN&mobileno=9643274071&message=aaa
     * 
     * //http://api.sparrowsms.com/v2/sms/?token=2Ca4N06OnVfBBl0BMemc&from=infoSMS
     * &to=9643274071&text=test
     * 
     * //System.out.println(" Mobile No -------------------->"+ phoneNo );
     * 
     * //System.out.println(" URL -------------------->"+ url_string );
     ...
     * //System.out.println(" Data -------------------->"+ message );
     * 
     * url = new URL( url_string ); URLConnection conn = url.openConnection();
     * conn.setDoOutput( true );
     * 
     * //sending data: OutputStreamWriter out = new OutputStreamWriter(
     * conn.getOutputStream() ); out.write( data ); out.flush();
     * 
     * //recieving response: InputStreamReader in = new InputStreamReader(
     * conn.getInputStream() ); BufferedReader buff_in = new BufferedReader( in
     * ); while ( buff_in.ready() ) { response += buff_in.readLine() + "   ";
     * //System.out.println( response + " " + data ); }
     * 
     * buff_in.close(); out.close();
     * 
     * return response; }
     */
