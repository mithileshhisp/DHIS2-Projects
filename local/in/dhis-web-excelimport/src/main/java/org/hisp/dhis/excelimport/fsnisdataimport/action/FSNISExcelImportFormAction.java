...
public class FSNISExcelImportFormAction implements Action
{
        ...
        try
        {
            ...
            for ( int s = 0; s < totalReports; s++ )
            {
                ...
                if ( reportNode.getNodeType() == Node.ELEMENT_NODE )
                {
                    ...
                    nodeList = reportElement.getElementsByTagName( "sheetNo" );
                    element = (Element) nodeList.item( 0 );
                    nodeList = element.getChildNodes();
                    sheetNo = Integer.parseInt(((Node) nodeList.item( 0 )).getNodeValue().trim());

                    BDImportSheet bdImportSheet = new BDImportSheet( xmlTemplateName, displayName, periodicity,checkerTemplateName, checkerRangeForHeader, checkerRangeForData,sheetNo);
