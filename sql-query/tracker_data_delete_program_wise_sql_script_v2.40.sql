...
-- 1) trackedentityattributevalueaudit
   ...
   ( select trackedentityinstanceid from programinstance where programid in ( select programid from 
   program where uid in ( 'ONV44BbaETD','ulfc9uQTQoi','Fcyldy4VqSt','uclDsv1MUXz', 'Soc1wdnWTEG'))); 
...
-- 3) relationship
	...
	relationshipitem where trackedentityinstanceid in ( select trackedentityinstanceid from programinstance where programid in ( select programid from 
   program where uid in ( 'ONV44BbaETD','ulfc9uQTQoi','Fcyldy4VqSt','uclDsv1MUXz', 'Soc1wdnWTEG')) )));
...
-- 5) trackedentitydatavalueaudit 
	...
	( select programstageinstanceid from programstageinstance where programstageid in 
		( select programstageid from programstage where programid in ( select programid from 
   program where uid in ( 'ONV44BbaETD','ulfc9uQTQoi','Fcyldy4VqSt','uclDsv1MUXz', 'Soc1wdnWTEG')) )
	);
...
-- 8) programinstancecomments 
	...
	( select programinstanceid from programinstance where programid in ( select programid from 
   program where uid in ( 'ONV44BbaETD','ulfc9uQTQoi','Fcyldy4VqSt','uclDsv1MUXz', 'Soc1wdnWTEG')) );

-- 9) programinstance 
      delete from programinstance where programid in ( select programid from 
   program where uid in ( 'ONV44BbaETD','ulfc9uQTQoi','Fcyldy4VqSt','uclDsv1MUXz', 'Soc1wdnWTEG')); 
