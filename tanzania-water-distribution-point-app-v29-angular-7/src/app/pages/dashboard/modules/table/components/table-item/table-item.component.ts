...
import { VisualizationExportService } from '../../../../../../core/services/visualization-export.service';
...
export class TableItemComponent implements OnInit {
  ...
  constructor(private tableService: TableService, private visualizationExportService: VisualizationExportService) {
  ...
  downloadTable(filename, downloadFormat) {
    if (this.table) {
      const el = this.table.nativeElement;
      if (downloadFormat === 'XLS') {
        if (el) {
          this.visualizationExportService.exportXLS(filename, el.outerHTML);
        }

      } else if (downloadFormat === 'CSV') {
        if (el) {
          this.visualizationExportService.exportCSV(filename, el);
        }
