...
export class SharingService {
  ...
  getUserList(): Observable<any> {
    return this.httpClient.get('users.json?fields=id,name,displayName&paging=false').pipe(
      map(res => res.users || []),
  ...
  getUserGroupList(): Observable<any> {
    return this.httpClient.get('userGroups.json?fields=id,name,displayName&paging=false').pipe(
