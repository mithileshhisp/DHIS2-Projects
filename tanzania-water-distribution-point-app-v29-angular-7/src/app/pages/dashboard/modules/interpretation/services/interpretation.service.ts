...
export class InterpretationService {
    ...
    return new Observable(observer => {
      ...
      this.httpClient.post(url, interpretationData.message).subscribe(() => {
        const interpretationsUrl = rootUrl + interpretationData.type + 's/' + interpretationData.id +
          '?fields=interpretations[id,type,text,lastUpdated,href,created,likes,likedBy[id,name,displayName],user' +
          '[id,name,displayName],comments[id,created,lastUpdated,text,user[id,name,displayName]],' +
          interpretationData.type + '[id,name]]';
        this.httpClient.get(interpretationsUrl).subscribe((interpretationResponse: any) => {
  ...
  edit(interpretation: any, rootUrl) {
    return new Observable(observer => {
      this.httpClient.put(rootUrl + '/interpretations/' + interpretation.id, interpretation.text).subscribe(() => {
        this.getInterpretation(interpretation, rootUrl).subscribe((interpretationRes) => {
          observer.next(interpretationRes);
          observer.complete();
        }, interpretationError => observer.error(interpretationError));
      }, error => observer.error(error));
    });
  ...
  updateLikeStatus(interpretation: any, rootUrl: string, like: boolean = true) {
    return new Observable(observer => {
      ...
      likePromise.subscribe(() => {
        this.getInterpretation(interpretation, rootUrl).subscribe((interpretation) => {
