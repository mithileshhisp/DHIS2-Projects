...
export class MessageConversationEffects {
  ...
  initializeMessageConvsersation$ = this.actions$.pipe(
    ofType(
      fromActions.MessageConversationActionTypes.INITIALIZE_MESSAGE_CONVERSATION
    ),
    withLatestFrom(this.store),
    tap(
      ([action, state]: [
        fromActions.InitializeMessageConversationAction,
        fromRoot.State
      ]) => {
        if (
          !state.messageConversation ||
          !state.messageConversation.loading ||
          !state.messageConversation.loaded
        ) {
  ...
  @Effect()
  loadMessageConversations$ = this.actions$.pipe(
    ofType(
      fromActions.MessageConversationActionTypes.LOAD_MESSAGE_CONVERSATION
    ),
    switchMap(() =>
      this.messageConversationService.loadAll().pipe(
        map(
          (messageConversations: MessageConversation[]) =>
            new fromActions.LoadMessageConversationSuccessAction(
              messageConversations
            )
        ),
        catchError(error =>
          of(new fromActions.LoadMessageConversationFailAction(error))
        )
      )
    )
  );
