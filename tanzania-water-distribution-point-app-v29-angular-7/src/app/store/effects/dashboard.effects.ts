...
import { getCheckedAddedItem } from '../../core/helpers/get-checked-added-item.helper';
...
import { getDashboardLoadedStatus } from '../selectors/dashboard.selectors';
import {
  DashboardActions,
...
export class DashboardEffects {
  ...
  currentUserLoaded$ = this.actions$.pipe(
    ofType<LoadCurrentUserSuccessAction>(CurrentUserActionTypes.LOAD_SUCCESS),
    tap(() => {
  ...
  loadDashboard$ = this.actions$.pipe(
    ...
    switchMap(([action, state]: [any, AppState]) =>
      this._loadAll().pipe(
        map(
          (dashboards: any[]) =>
            new LoadDashboardSuccessAction({
              dashboards,
              currentUser: state.currentUser ? state.currentUser.user : null,
              url: state.route && state.route.state ? state.route.state.url : ''
            })
        ),
  ...
  @Effect({ dispatch: false })
  dashboardsLoaded$ = this.actions$.pipe(
    ofType<LoadDashboardSuccessAction>(DashboardActions.LOAD_DASHBOARD_SUCCESS),
    tap(() => {
  ...
  @Effect({ dispatch: false }) globalFilter$ = this.actions$.pipe(
    ofType(GlobalFilterActionTypes.ADD_GLOBAL_FILTER),
    withLatestFrom(this.store),
    tap(([action, state]: [AddGlobalFilterAction, AppState]) => {
      this.store
        .select(getDashboardLoadedStatus)
        .pipe(filter((loadedStatus: boolean) => loadedStatus))
        .subscribe(() => {
          const splitedUrl = state.route.state.url.split('/');
          const page = splitedUrl[1];

          if (page && page === 'dashboards') {
            this.store.dispatch(
              new dashboard.SetCurrentAction(splitedUrl[2].split('?')[0])
            );
          }
        });
    })
  ...
  route$ = this.actions$.pipe(
    ...
    tap(([action, state]: [any, AppState]) => {
      const dashboardIdSection = state.route.state.url.split('/')[2];
      const currentDashboardId = dashboardIdSection
        ? dashboardIdSection.split('?')[0]
        : '';
      ...
      if (currentDashboardId) {
        ...
        if (currentDashboard && currentGlobalFilter) {
          /**
           * Save current dashboard to local storage
           */
          localStorage.setItem(
            'dhis2.dashboard.current.' +
              state.currentUser.user.userCredentials.username,
            currentDashboardId
          );
  ...
  dashboardItemAddedAction$ = this.actions$.pipe(
    ...
    switchMap(([action, state]: [any, AppState]) => {
      ...
      if (currentDashboard) {
        const newDashboardItem: any = getCheckedAddedItem(
          currentDashboard,
          action.payload
        );

        const initialVisualization: Visualization = mapDashboardItemToVisualization(
          newDashboardItem,
          state.dashboard.currentDashboard,
  ...
  loadDashboardNotification$ = this.actions$.pipe(
    ...
    switchMap(() =>
      this.httpClient.get('me/dashboard.json').pipe(
        map(
          (response: any) =>
