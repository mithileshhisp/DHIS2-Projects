...
export function constructAnalyticsUrl(
  ...
  const aggregationType: string = visualizationSettings.hasOwnProperty('aggregationType')
    ? visualizationSettings.aggregationType !== 'DEFAULT'
      ? '&aggregationType=' + visualizationSettings.aggregationType
      : ''
    : '';
  ...
  const parameters = getParametersArray(visualizationFilters, vizType).join('&');
...
function getParametersArray(filters: any[], vizType: string = 'CHART') {
  return _.filter(
    _.map(filters, filter => {
      return filter.value !== ''
        ? (filter.layout === 'filter' && vizType !== 'MAP' ? 'filter=' : 'dimension=') +
            filter.name +
            ':' +
            filter.value
        : ['dx', 'pe', 'ou'].indexOf(filter.name) === -1
        ? (filter.layout === 'filter' && vizType !== 'MAP' ? 'filter=' : 'dimension=') + filter.name
        : '';
    }),
    param => param !== ''
  );
