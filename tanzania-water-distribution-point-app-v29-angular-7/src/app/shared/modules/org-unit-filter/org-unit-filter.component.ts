...
export class OrgUnitFilterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription = this.orgunitService.getOrgunitLevelsInformation().subscribe((data: any) => {
      ...
      this.subscription =
        this.orgunitService.getUserInformation(this.orgUnitModel.type).subscribe((userOrgunit: any) => {
          if (userOrgunit !== null) {
            ...
            const use_level = parseInt(all_levels, 10) - (parseInt(level, 10) - 1) - 1;
            ...
            this.subscription = this.orgunitService.getInitialOrgunitsForTree(orgunits).subscribe(
              (initial_data) => {
                ...
                this.subscription =
                  this.orgunitService.getAllOrgunitsForTree1(fields, orgunits).subscribe((items: any[]) => {
                      this.organisationunits = items.length > 0 ? this.sortOrganisationUnitTree([
                        {
                          ...items[0],
                          isExpanded: true
                        },
                      ...
                      for (const active_orgunit of this.orgUnitModel.selectedOrgUnits.filter(orgUnit => orgUnit)) {
                        this.activateNode(active_orgunit.id,
                          this.orgtree, active_orgunit.id === this.orgUnitModel.userOrgUnits.id);
  ...
  activateOrg($event) {
    ...
    _.each(this.selectedOrgUnits, selectedOrgUnit => {
      allowedLevels = [
        ...allowedLevels,
        ..._.filter(this.orgUnitModel.selectedLevels, selectedLevel => selectedLevel.level > selectedOrgUnit.level)
      ];
    });
