...
excelUpload.controller('AddTemplateController',
	...
	userService.getCurrentUser().then(function (responseUser){
        ...
        $scope.getDataSetInfo = function(){
            ...
            var url = "../api/dataSets/"+ $rootScope.selectedDataSet +".json?fields=id,organisationUnits[id],dataSetElements[dataElement[id,name,shortName,categoryCombo[categoryOptionCombos[id,name]]]]&paging=false";
            $.get(url , function(ds){
                $scope.selectedDataSetInfo = ds;

                var s = $scope.selectedDataSetInfo.dataSetElements.length;
                ...
                for( var x = 0; x < s ; x++ )
                {
                    ...
                    {
                        var t = {};
                        if( $scope.selectedDataSetInfo.dataSetElements[y].name > $scope.selectedDataSetInfo.dataSetElements[y+1].name)
                        {
                            t = $scope.selectedDataSetInfo.dataSetElements[y];
                            $scope.selectedDataSetInfo.dataSetElements[y] = $scope.selectedDataSetInfo.dataSetElements[y+1] ;
                            $scope.selectedDataSetInfo.dataSetElements[y+1] = t;
                        }
                    }
                }
