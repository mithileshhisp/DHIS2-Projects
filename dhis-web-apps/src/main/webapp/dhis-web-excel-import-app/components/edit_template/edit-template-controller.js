    ...
    function($rootScope,
        ...
        $scope.getSelectedDSWithCOC = function(){
            ...
            $.get(url , function(ds){
                ...
                var s = $scope.selectedDSWithCOC.dataSetElements.length;
                ...
                for( var x = 0; x < s ; x++ )
                {
                    ...
                    {
                        var t = {};
                        if( $scope.selectedDSWithCOC.dataSetElements[y].name > $scope.selectedDSWithCOC.dataSetElements[y+1].name)
                        {
                            t = $scope.selectedDSWithCOC.dataSetElements[y];
                            $scope.selectedDSWithCOC.dataSetElements[y] = $scope.selectedDSWithCOC.dataSetElements[y+1] ;
                            $scope.selectedDSWithCOC.dataSetElements[y+1] = t;
                        }
                    }
                }

                $scope.selectedDSWithCOC.dataSetElements.forEach(function(de){
                    de.dataElement.categoryCombo.categoryOptionCombos.forEach(function(coc){
                        $scope.availableOptions.push({id:de.dataElement.id + "-" + coc.id, name:de.dataElement.name + " - " + coc.name});
                    });
                });
                console.log("34433");
