...
trackerCapture.controller('RegistrationController', 
    ...
    // CCEI Related declare
    $scope.trackerAssociateInstances = [];
    ...
    if(!$scope.attributesById){
     ...
     CCEIProgram.getModelAttributes().then(function(modelAttributes){
        $scope.modelAttributes = modelAttributes;
        //console.log( "1--" + $scope.modelAttributes.length);
    });


// CCEI Related methods
   if ($scope.selectedProgram != undefined)
   {
       if ($scope.trackerAssociateInstances.length == 0)
       {
                       ...
                       if (att.valueType == 'TRACKER_ASSOCIATE') {
                           CCEIConfigurationSetting.getMappedProgram($scope.selectedProgram.id).then(function(modelProgramId){
                               CCEIProgram.getTrackedEntityInstancesByProgram(modelProgramId, ou.id).then(function (teis) {
                                   //console.log( "2--" + $scope.modelAttributes.length);
                                   for (var i in teis.trackedEntityInstances) {

                                       var spaceSeparatedAttributeValues = "";

                                       for (var j in teis.trackedEntityInstances[i].attributes) {

                                           for (var k=0;k<$scope.modelAttributes.length;k++ )
                                           {
                                               if( teis.trackedEntityInstances[i].attributes[j].attribute == $scope.modelAttributes[k].id)
                                               {
                                                   spaceSeparatedAttributeValues = spaceSeparatedAttributeValues + teis.trackedEntityInstances[i].attributes[j].value + " ";
                                               }
                                           }

                                           /*
                                           if( teis.trackedEntityInstances[i].attributes[j].displayName == 'Catalog ID' || teis.trackedEntityInstances[i].attributes[j].displayName == 'Manufacturer'
                                               || teis.trackedEntityInstances[i].attributes[j].displayName == 'Model' )
                                           {
                                               spaceSeparatedAttributeValues = spaceSeparatedAttributeValues + teis.trackedEntityInstances[i].attributes[j].value + " ";
                                           }
                                           */
                                           ...
                                           //spaceSeparatedAttributeValues = spaceSeparatedAttributeValues + teis.trackedEntityInstances[i].attributes[j].value + " ";
                                           //console.log( teis.trackedEntityInstances[i].trackedEntityInstance +" -- " + spaceSeparatedAttributeValues );
                                       }

                                       var tEICustomObject = {
                                           "id" : teis.trackedEntityInstances[i].trackedEntityInstance,
                                           "value" : spaceSeparatedAttributeValues
                                       }
                                       $scope.trackerAssociateInstances.push(tEICustomObject);
                                       $scope.trackerAssociateInstancesMap[tEICustomObject.id] = tEICustomObject;
                                       //console.log( teis.trackedEntityInstances[i].trackedEntityInstance +" -- " + spaceSeparatedAttributeValues );
                                   }
                                   ...
                                   for (var count in teis.rows) {
                                       ...
                                       var spaceSeparatedAttributeValues = "";
                                       for (var i = 5; i < tei.length; i++) {
                                           spaceSeparatedAttributeValues = spaceSeparatedAttributeValues + tei[i] + " ";
                                       }
