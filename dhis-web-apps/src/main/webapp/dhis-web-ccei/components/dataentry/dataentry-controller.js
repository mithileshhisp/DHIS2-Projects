...
trackerCapture.controller('DataEntryController',
    ...
    $scope.saveDatavalueForEvent = function (prStDe, field, eventToSave) {
        ...
        if (oldValue !== value) {
            ...
            // for CCEI save dataElement value to Attribute
            if( prStDe.dataElement.id == $rootScope.ConfigurationSettings.parameters['working-status-data-element'].value )
            {
                var wsAttributeUid = $rootScope.ConfigurationSettings.parameters['working-status-attribute'].value;
                var updateResponse = CCEIUpdateAttributeService.updateAttributeValue( $scope.currentEvent.trackedEntityInstance, wsAttributeUid, value, $scope.optionSets, $scope.attributesById);
